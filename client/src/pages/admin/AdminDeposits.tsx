...
export default function AdminDeposits() {
  ...
  const { data: deposits, isLoading, refetch } = useQuery<Transaction[]>({
    queryKey: ['/api/admin/transactions/deposits'],
  ...
  const handleApproveDeposit = async (id: number) => {
    try {
      ...
      toast({
        ...
        description: "The deposit has been approved and processed.",
      });
      refetch();
    } catch (error) {
      toast({
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => handleApproveDeposit(transaction.id)}
  ...
  const totalPendingAmount = pendingDeposits.reduce((sum, d) => sum + parseFloat(d.amount), 0);
  const totalDepositedAmount = completedDeposits.reduce((sum, d) => sum + parseFloat(d.amount), 0);
          ...
          <div className="max-w-7xl mx-auto">
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-2xl font-bold">
                        {isLoading ? <span className="animate-pulse">Loading...</span> : pendingDeposits.length}
                      </p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-2xl font-bold">
                        {isLoading ? <span className="animate-pulse">Loading...</span> : formatCurrency(totalDepositedAmount, 'USDT')}
                      </p>
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                ) : [...completedDeposits, ...rejectedDeposits].length > 0 ? (
                  <DataTable 
                    columns={columns} 
                    data={[...completedDeposits, ...rejectedDeposits]} 
                    searchColumn="user"
                    searchPlaceholder="Search by user..."
                  />
                ) : (
                  <div className="text-center py-8">
                    <p className="text-muted-foreground">No processed deposits</p>
                  </div>
                )}
              </CardContent>
            </Card>
